...
Üye.roles.add(ayarlar.GeliştiriciRolü)
message.guild.members.cache.get(Client).roles.add(ayarlar.BOTRol)
message.guild.channels.cache.get(ayarlar.BOTLog).send(`<@${Üye.id}> Adlı Kullanıcının \`${User.tag}\` Adlı Botu ${message.author} Tarafından Onaylandı!`)
client.users.cache.get(Üye.id).send(`\`${message.guild.name}\` Adlı Sunucuda ${message.author.tag}(\`${message.author.id}\`) Tarafından \`${User.tag}\` Botun Onaylandı`)
message.react('✅')
})
}
